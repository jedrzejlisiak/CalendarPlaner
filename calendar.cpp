...
void Calendar::hide(int year, int xmonth, int xday){
    this->middle();
    string kname;
    kname = name;
    for(int i =0; i<100;i++)
...
void Calendar::init(){
    ...
    backend->bind("#nano#<F8>%Hide schedule!Type layer name:${NAMEE}",[&](){hide(year, xmonth, xday);}, "Hide schedule by its name");
}